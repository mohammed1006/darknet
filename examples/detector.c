...
//  printf("%sf\n",ftp_name);
		...
		//char ftp_path[100];
		//sprintf(ftp_path,"%s\n",ftp_path1);
		float ftp_thresh = 0.01 * option_find_int(options2, "ftp_thresh", 0);
...
//  printf("%sf\n",ftp_pwd);
		...
		Args_MY arg = {server, port, robotID, ftp_thresh};

		if (pthread_create(&socket_thread, 0, setupSocketWarp, (void*)&arg))printf("create fail\n");
