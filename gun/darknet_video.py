...
def YOLO():
    ...
    if netMain is None:
        netMain = load_net_custom(configPath.encode(
            "ascii"), weightPath.encode("ascii"), 0,2)  # batch size = 1
    if metaMain is None:
    ...
    # img2=cv2.imread("maks.jpeg")
    # img1=cv2.resize(img1,(416,416))
    # img2=cv2.resize(img2,(416,416))
    # com=np.stack([img1,img2])
    # print(com.shape)
    # #detections=cv2.resize(frame_rgb,(lib.network_width(netMain),lib.network_height(netMain)),interpolation=cv2.INTER_LINEAR)
    # detections = detect(netMain, metaMain, com, thresh=0.25)
