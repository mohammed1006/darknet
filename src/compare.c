...
int bbox_comparator(const void *a, const void *b)
{
    ...
    float* X = (float*)xcalloc(net.w * net.h * net.c, sizeof(float), __FILE__, __LINE__);
...
void SortMaster3000(char *filename, char *weightfile)
{
    ...
    int N = plist->size;
    free_list(plist);
    sortable_bbox* boxes = (sortable_bbox*)xcalloc(N, sizeof(sortable_bbox), __FILE__, __LINE__);
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < N; ++i){
        ...
        boxes[i].elos = (float*)xcalloc(classes, sizeof(float), __FILE__, __LINE__);
        for(j = 0; j < classes; ++j){
            boxes[i].elos[j] = 1500;
        }
    }
