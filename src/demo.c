...
static int demo_save_json_output = 0;
static int demo_save_image_output = 0;
static char *demo_output_prefix[256];
...
void *detect_in_thread(void *ptr)
{
	...
	*/
	...
	draw_detections_cv_v3(det_img, dets, nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output, demo_save_json_output, demo_save_image_output, demo_output_prefix);
	...
	int frame_skip, char *prefix, char *out_filename, int http_stream_port, int dont_show, int ext_output, int save_json_output, int save_image_output, char *output_prefix)
{
	...
	demo_save_json_output = save_json_output;
	demo_save_image_output = save_image_output;

	if (demo_save_json_output || demo_save_image_output) {
		if (output_prefix == NULL) {
			strcpy(demo_output_prefix, filename);
		}
		else {
			strcpy(demo_output_prefix, output_prefix);
		}
	}
