...
layer make_gru_layer(int batch, int inputs, int outputs, int steps, int batch_normalize)
{
    ...
    l.input_z_layer = (layer*)xcalloc(1,sizeof(layer), __FILE__, __LINE__);
    fprintf(stderr, "\t\t");
    *(l.input_z_layer) = make_connected_layer(batch, steps, inputs, outputs, LINEAR, batch_normalize);
    l.input_z_layer->batch = batch;

    l.state_z_layer = (layer*)xcalloc(1,sizeof(layer), __FILE__, __LINE__);
    fprintf(stderr, "\t\t");
    *(l.state_z_layer) = make_connected_layer(batch, steps, outputs, outputs, LINEAR, batch_normalize);
    ...
    l.input_r_layer = (layer*)xcalloc(1,sizeof(layer), __FILE__, __LINE__);
    fprintf(stderr, "\t\t");
    *(l.input_r_layer) = make_connected_layer(batch, steps, inputs, outputs, LINEAR, batch_normalize);
    l.input_r_layer->batch = batch;

    l.state_r_layer = (layer*)xcalloc(1,sizeof(layer), __FILE__, __LINE__);
    fprintf(stderr, "\t\t");
    *(l.state_r_layer) = make_connected_layer(batch, steps, outputs, outputs, LINEAR, batch_normalize);
    ...
    l.input_h_layer = (layer*)xcalloc(1,sizeof(layer), __FILE__, __LINE__);
    fprintf(stderr, "\t\t");
    *(l.input_h_layer) = make_connected_layer(batch, steps, inputs, outputs, LINEAR, batch_normalize);
    l.input_h_layer->batch = batch;

    l.state_h_layer = (layer*)xcalloc(1,sizeof(layer), __FILE__, __LINE__);
    fprintf(stderr, "\t\t");
    *(l.state_h_layer) = make_connected_layer(batch, steps, outputs, outputs, LINEAR, batch_normalize);
    ...
    l.delta = (float*)xcalloc(outputs * batch * steps, sizeof(float), __FILE__, __LINE__);
    l.state = (float*)xcalloc(outputs * batch, sizeof(float), __FILE__, __LINE__);
    l.prev_state = (float*)xcalloc(outputs * batch, sizeof(float), __FILE__, __LINE__);
    l.forgot_state = (float*)xcalloc(outputs * batch, sizeof(float), __FILE__, __LINE__);
    l.forgot_delta = (float*)xcalloc(outputs * batch, sizeof(float), __FILE__, __LINE__);

    l.r_cpu = (float*)xcalloc(outputs * batch, sizeof(float), __FILE__, __LINE__);
    l.z_cpu = (float*)xcalloc(outputs * batch, sizeof(float), __FILE__, __LINE__);
    l.h_cpu = (float*)xcalloc(outputs * batch, sizeof(float), __FILE__, __LINE__);
