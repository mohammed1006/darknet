...
local_layer make_local_layer(int batch, int h, int w, int c, int n, int size, int stride, int pad, ACTIVATION activation)
{
    ...
    l.weights = (float*)xcalloc(c * n * size * size * locations, sizeof(float), __FILE__, __LINE__);
    l.weight_updates = (float*)xcalloc(c * n * size * size * locations, sizeof(float), __FILE__, __LINE__);

    l.biases = (float*)xcalloc(l.outputs, sizeof(float), __FILE__, __LINE__);
    l.bias_updates = (float*)xcalloc(l.outputs, sizeof(float), __FILE__, __LINE__);

    // float scale = 1./sqrt(size*size*c);
    float scale = sqrt(2./(size*size*c));
    for(i = 0; i < c*n*size*size; ++i) l.weights[i] = scale*rand_uniform(-1,1);

    l.col_image = (float*)xcalloc(out_h * out_w * size * size * c, sizeof(float), __FILE__, __LINE__);
