...
//列表
int ftp_list_n( int c_sock, char *path, void **data, unsigned long long *data_len)
{
	int     d_sock;
	char    buf[512];
	int     send_re;
...
int ftp_storefile_data(int c_sock, char *data, char* name, unsigned long long size)
{
	int     d_sock;
	ssize_t     len;
	char    buf[512];
	...
	while (i < sum)
	{
		...
		if (write(d_sock, &data[i], nread) != nread)
		{
