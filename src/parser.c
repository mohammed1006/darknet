...
//#ifdef UNIFIED_MEM
//        cudaMallocManaged(&mask, n*sizeof(int), cudaMemAttachGlobal);
//#else
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    {
        printf("global_layer creation! %d\n",max_size_of_nweights);
        hGlobal_layer_weights = (float*)xmalloc(max_size_of_nweights*sizeof(float));
//        cudaMallocHost((float *)&hGlobal_layer_weights, max_size_of_nweights*sizeof(float));
        cudaMallocHost((float *)&hhGlobal_layer_weights, max_size_of_nweights*sizeof(float));
        cudaMalloc(&global_layer_weights,max_size_of_nweights*sizeof(float));
...
void load_convolutional_weights(layer *l, FILE *fp)
{
    ...
    fseek(l->weights_file, num*sizeof(float), SEEK_CUR); // just move
#else
