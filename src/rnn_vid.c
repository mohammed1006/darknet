...
float_pair get_rnn_vid_data(network net, char **files, int n, int batch, int steps)
{
    ...
    float* feats = (float*)xcalloc(net.batch * batch * output_size, sizeof(float), __FILE__, __LINE__);
    for(b = 0; b < batch; ++b){
        int input_size = net.w*net.h*net.c;
        float* input = (float*)xcalloc(input_size * net.batch, sizeof(float), __FILE__, __LINE__);
